...
2026-03-18
- Room seats are now "seatCount" based: host can add empty seats up to 5, and can only remove the last empty seat (no kicking joined players). Game start requires >=2 players and no empty seats.
- Setup no longer auto-fills leftover reserves on lock; players must place all reserves manually.
- Setup supports abandoning a starting territory (checkbox) so it starts unoccupied (white, 0 units).
- Frontend: unoccupied territory text color fixed for contrast.
- Frontend: added zh/en language toggle (default zh) and translated most UI labels; backend logs remain English.
- Verified builds: `backend mvn -DskipTests package`, `frontend npm test`, `frontend npm run build`. Installed Playwright Chromium locally via `npx playwright install chromium` for future UI smoke tests.
- Replaced circle-based map rendering with polygon territories (Voronoi cells). Backend now generates Voronoi polygons and neighbor graph via JTS, and exposes vertices in `TerritoryView.polygon`. Frontend draws and hit-tests polygons when present.
